...
The web app keeps its register in JSON files beside it (`.data/`). The skill
keeps one in a **Google Sheet on Drive**. They are separate copies, not one
shared store — so a grant recorded in the sheet does not appear in the app, and
vice versa.

That is worth stating out loud whenever it matters. If the app is reachable,
its API is the better path: it does the provider call and the record in one
step, and it enforces the approval rules in code. Use the sheet when it is not.
...
## Publishing the app's register to the Sheet

```
POST /api/sync   {spreadsheetId?}   -> {ok, detail, written, url, tasksUsed}
```

Copies the app's register into a Google Sheet, one tab per collection — the
same shape the skill reads. Use it to bring the sheet back in step with the app
after the app has been doing the work.
